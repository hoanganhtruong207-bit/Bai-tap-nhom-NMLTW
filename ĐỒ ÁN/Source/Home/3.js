...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // =========================================================================
    const headerLinks = document.querySelectorAll(".horizontal-nav__link, .login");
    ...
    // Khởi chạy vòng lặp đổi ảnh nếu tồn tại phần tử trên giao diện
    if (movieBanners.length > 0) {
        renderActiveBanner(activeBannerIndex);

        bannerInterval = setInterval(() => {
            activeBannerIndex = (activeBannerIndex + 1) % movieBanners.length;
            renderActiveBanner(activeBannerIndex);
        }, 3000); // 3000ms = Cứ mỗi 3 giây tự động chuyển sang phim kế tiếp
    }
    ...
    function executeMovieSearch() {
        ...
        if (matchCount === 0) {
            alert(`Rạp không tìm thấy siêu phẩm nào khớp với từ khóa: "${searchField.value}"`);
        }
    }
    ...
    movieItems.forEach(item => {
        ...
        if (clickableTitle) {
            ...
            clickableTitle.addEventListener("click", () => {
                ...
                alert(`Hệ thống ghi nhận: Bạn đã chọn phim "${textMovieName}". Hãy sang trang lịch chiếu/đặt vé!`);
            });
        }
    });
});